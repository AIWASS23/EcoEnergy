...
class Extractor {
    ...
    private static func extractFromDocx(url: URL) async throws -> String {
        ...
        do {
            ...
            let textExtractorDirUrl: URL = FileSystem.getAppSupportDirUrl().appendingPathComponent("Text Extractor")
            FileSystem.createDirectory(url: textExtractorDirUrl)
            let textExtractorDirPath: String = textExtractorDirUrl.posixPath()
            let _: String = CLI.runCommand(command: "cd \"\(textExtractorDirPath)\"; rm -rf \"wordDecodeContents/\(archiveId.uuidString)\"; mkdir -p \"wordDecodeContents/\(archiveId.uuidString)\"; cd \"wordDecodeContents/\(archiveId.uuidString)\"; unzip \"\(filepath)\"")

            let mediaDirUrl: URL = FileSystem.getAppSupportDirUrl().appendingPathComponent("Text Extractor").appendingPathComponent("wordDecodeContents").appendingPathComponent(archiveId.uuidString).appendingPathComponent("word").appendingPathComponent("media")

            var nsImages: [NSImage] = []
            let _: [URL] = FileSystem.listDirectory(dirUrl: mediaDirUrl)
            for currFile in FileSystem.listDirectory(dirUrl: mediaDirUrl) {
                if Extractor.isRecognizedImage(url: currFile) {
            ...
            let _ = CLI.runCommand(command: "rm -rf \"\(textExtractorDirPath)/wordDecodeContents/\(archiveId.uuidString)\"")
            return fileString
    ...
    private static func extractFromPptx(url: URL) async throws -> String {
        ...
        FileSystem.createDirectory(url: textExtractorDirUrl)
        let textExtractorDirPath: String = textExtractorDirUrl.posixPath()
        let cliResult: String = CLI.runCommand(command: "cd \"\(textExtractorDirPath)\"; rm -rf \"pptDecodeContents/\(archiveId.uuidString)\"; mkdir -p \"pptDecodeContents/\(archiveId.uuidString)\"; cd \"pptDecodeContents/\(archiveId.uuidString)\"; unzip \"\(filepath)\"; cd ppt/slides; echo \"-<>-<>-separator-<>-<>-\"; grep -r -h -E -o '<a:t>[^<]*</a:t>' . | sed -E 's/<[^>]*>//g' | grep -v '^./slide.*'")
        // Kill lingering process
        let _ = CLI.runCommand(command: "killall grep")
        // Check if no error
        if cliResult.contains("-<>-<>-separator-<>-<>-\n") {
            ...
            let mediaDirUrl: URL = FileSystem.getAppSupportDirUrl().appendingPathComponent("Text Extractor").appendingPathComponent("pptDecodeContents").appendingPathComponent(archiveId.uuidString).appendingPathComponent("ppt").appendingPathComponent("media")
            ...
            let _ = CLI.runCommand(command: "rm -rf \"\(textExtractorDirPath)/pptDecodeContents/\(archiveId.uuidString)\"")
            return fileText
        } else {
            // Delete unzipped archive folder
            let _ = CLI.runCommand(command: "rm -rf \"\(textExtractorDirPath)/pptDecodeContents/\(archiveId.uuidString)\"")
    ...
    private static func extractFromPages(url: URL) async throws -> String {
        ...
        FileSystem.createDirectory(url: textExtractorDirUrl)
        let textExtractorDirPath: String = textExtractorDirUrl.posixPath()
        let _: String = CLI.runCommand(command: "cd \"\(textExtractorDirPath)\"; rm -rf \"pagesDecodeContents/\(archiveId.uuidString)\"; mkdir -p \"pagesDecodeContents/\(archiveId.uuidString)\"; cd \"pagesDecodeContents/\(archiveId.uuidString)\"; unzip \"\(filepath)\"")
        // Get image text in ~/Library/Application Support/Magic Sorter/pagesDecodeContents/archiveId
        let mediaUrl: URL = FileSystem.getAppSupportDirUrl().appendingPathComponent("Text Extractor").appendingPathComponent("pagesDecodeContents").appendingPathComponent(archiveId.uuidString).appendingPathComponent("preview.jpg")
        // Get image
        if Extractor.isRecognizedImage(url: mediaUrl) {
            guard let nsImage: NSImage = NSImage(contentsOf: mediaUrl) else { throw ExtractionError.decodeError }
            // Extract text
            do {
                let extractedText: String = try extractFromNSImage(nsImage: nsImage)
                // Delete unzipped archive folder
                let _ = CLI.runCommand(command: "rm -rf \"\(textExtractorDirPath)/pagesDecodeContents/\(archiveId.uuidString)\"")
